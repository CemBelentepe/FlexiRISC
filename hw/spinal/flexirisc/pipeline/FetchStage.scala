...
case class IfId1() extends Component {
  val io = new Bundle {
    ...
    val if_pc = in UInt (64 bits)
    val if_pc_next_seq = in UInt(64 bits)

    val id1_pc = out UInt(64 bits)
    val id1_pc_next_seq = out UInt(64 bits)
  }
...
case class FetchStage() extends Component {
  val io = new Bundle {
    ...
    val jump_address = in UInt(64 bits)
    val halt_stage = in Bool()

    val pc = out UInt (64 bits)
    val pc_next_seq = out UInt(64 bits)
