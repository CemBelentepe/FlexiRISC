...
case class WriteBackStage() extends Component{
  ...
  when(io.control_signals.is_load) {
    io.data := io.control_signals.funct3.mux(
      ...
      B"3'b001" -> io.wb_load_data.subdivideIn(16 bits)(io.wb_result_data(1).asUInt).asSInt.resize(32 bits).asBits,
      ...
      B"3'b101" -> io.wb_load_data.subdivideIn(16 bits)(io.wb_result_data(1).asUInt).asUInt.resize(32 bits).asBits,
      default -> B"32'b0"
    )
  }otherwise {
